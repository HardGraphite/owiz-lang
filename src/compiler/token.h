...
ow_static_inline void ow_token_fini(struct ow_token *tok) {
	if (tok->_type == OW_TOK_STRING || tok->_type == OW_TOK_IDENTIFIER)
...
ow_static_forceinline struct ow_sharedstr *ow_token_value_string(
		const struct ow_token *tok) {
	assert(tok->_type == OW_TOK_STRING || tok->_type == OW_TOK_IDENTIFIER);
	return tok->_data.s;
}
...
ow_static_inline void ow_token_assign_string(
	...
	assert(tp == OW_TOK_STRING || tp == OW_TOK_IDENTIFIER);
	ow_token_fini(tok);
	tok->_type = tp;
